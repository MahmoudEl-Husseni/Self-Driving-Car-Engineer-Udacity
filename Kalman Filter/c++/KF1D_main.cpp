...
void filter(VectorXd &x, MatrixXd &P) {
    for (auto z : measurements) {
        cout << z << endl;
        // Predict the state and covariance matrix using the process model and the previous state and covariance matrix
        x = F * x + u;
        P = F * P * F.transpose() + Q;

        // Update the state and covariance matrix using the measurement model and the previous state and covariance matrix
        VectorXd y = z - H * x;
